...
CREATE TABLE junctions (time SERIAL PRIMARY KEY NOT NULL, con1 INTEGER NOT NULL, con2 INTEGER NOT NULL);
...
DECLARE circuit_id2 INTEGER;
BEGIN
    FOR circuit_id1, circuit_id2 IN SELECT circuits1.id, circuits2.id FROM circuits AS circuits1
    ...
    LOOP
        PERFORM circuit_connect(circuit_id1, circuit_id2);
        INSERT INTO junctions(con1, con2) SELECT circuit_id1, circuit_id2;
        EXIT WHEN EXISTS (SELECT 1 FROM (SELECT count(*) FROM circuits WHERE circuits.con IS NULL) AS unconnected(count) WHERE unconnected.count = 1);
    END LOOP;
END;
$$;

SELECT circuits1.pos_x * circuits2.pos_x FROM junctions
    LEFT JOIN circuits AS circuits1 ON junctions.con1 = circuits1.id
    LEFT JOIN circuits AS circuits2 ON junctions.con2 = circuits2.id
    ORDER BY junctions.time DESC
    LIMIT 1;
