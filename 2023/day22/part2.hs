...
fallOne :: Brick -> [Brick] -> (Brick, Bool)
fallOne brick@((_, _, z), _) fallen
  | z <= 1 = (brick, False)
  | all (not . occupy down) fallen = (fst $ fallOne down fallen, True)
  | otherwise = (brick, False)
  where down = moveDown brick

fallDown :: [Brick] -> [Brick] -> [Brick]
fallDown [] fallen = fallen
fallDown (head : tail) fallen = fallDown tail ((fst $ fallOne head fallen) : fallen)

minorLists :: [a] -> [[a]]
minorLists [] = []
minorLists (head : tail) = tail : map (head :) (minorLists tail)

countFalling :: [Brick] -> [Brick] -> Int
countFalling [] fallen = 0
countFalling (head : tail) fallen = (+) (fromEnum didFall) $ countFalling tail (new : fallen) where (new, didFall) = fallOne head fallen

main = do
  bricks <- sortBricks <$> map parseBrick <$> lines <$> getContents
  fallen <- return $ sortBricks $ fallDown bricks []
  putStrLn $ show $ sum $ map (\b -> countFalling b []) $ minorLists fallen
