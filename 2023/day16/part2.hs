...
entryPoints :: (Int, Int) -> [(Position, Direction)]
entryPoints (w, h) = [(Position 0 y, East) | y <- [0..h-1]] ++ [(Position (w-1) y, West) | y <- [0..h-1]] ++ [(Position x 0, South) | x <- [0..w-1]] ++ [(Position x (h-1), North) | x <- [0..w-1]]
...
main = do
  ...
  allLightMaps <- return $ map (\(p,d) -> trace area Map.empty (width, height) p d) $ entryPoints (width, height)
  putStrLn $ show $ maximum $ map (length . Map.keys) allLightMaps
