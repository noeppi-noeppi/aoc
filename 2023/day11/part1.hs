...
moveRight Position{getX = x, getY = y} = Position (x+1) y
moveDown Position{getX = x, getY = y} = Position x (y+1)

expandLine :: (Position -> Int) -> (Position -> Position) -> Image -> Int -> Image
expandLine coordinate move image lineIdx = map (\pos -> if (coordinate pos > lineIdx) then move pos else pos) image

maybeExpandLine :: (Position -> Int) -> (Position -> Position) -> Image -> Int -> Image
...
expandImageBy :: (Position -> Int) -> (Position -> Position) -> Image -> Image
