...
while lastSize < mayContain.count {
    lastSize = mayContain.count
    for rule in rules {
        for c in rule.contain {
            if (!mayContain.contains(rule.type) && (mayContain.contains(c.type) || c.type == "shiny gold")) {
                mayContain.append(rule.type)
            }
        }
    }
}

print(mayContain.count)