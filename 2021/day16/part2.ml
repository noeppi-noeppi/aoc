...
class literal (f_ver: int) (f_type: int) (f_value : int) = object (self) inherit packet
  method value : int = f_value
end;;

class operator (f_ver: int) (f_type: int) (f_mode : bool) (f_args : packet list) = object (self) inherit packet
  method value : int = match f_type with
    | 0 -> List.fold_left ( + ) 0 (List.map (fun p -> p#value) f_args)
    | 1 -> List.fold_left ( * ) 1 (List.map (fun p -> p#value) f_args)
    | 2 -> List.fold_left (fun a b -> if a < b then a else b) Int.max_int (List.map (fun p -> p#value) f_args)
    | 3 -> List.fold_left (fun a b -> if a > b then a else b) 0 (List.map (fun p -> p#value) f_args)
    | 5 -> if (List.hd f_args)#value > (List.hd (List.tl f_args))#value then 1 else 0
    | 6 -> if (List.hd f_args)#value < (List.hd (List.tl f_args))#value then 1 else 0
    | 7 -> if (List.hd f_args)#value = (List.hd (List.tl f_args))#value then 1 else 0
    | n -> 0
end;;
