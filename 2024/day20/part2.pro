...
main :-
  load,
  iterate_path(Path),
  maplist(find_cheats_once, Path, NestedCheats),
...
find_cheats_once(From, Cheats) :- once(find_cheats(From, Cheats)).

find_cheats(From, Cheats) :-
  offsets20(From, Offsets),
  exclude(not_grid, Offsets, GridOffsets),
  exclude(not_back_to(From), GridOffsets, SkipTargets),
...
neighbours([X, Y], Neighbours) :- offsets([X, Y], Offsets), exclude(obstructed, Offsets, Neighbours).
not_grid([X, Y]) :- width(W), height(H), (X < 0 ; X >= W ; Y < 0 ; Y >= H).
obstructed([X, Y]) :- width(W), height(H), (X < 0 ; X >= W ; Y < 0 ; Y >= H ; cell(X, Y, wall)).

manhattan([XO, YO], [XN, YN], Dist) :- DX is abs(XN - XO), DY is abs(YN - YO), Dist is DX + DY.
manhattan20([XO, YO], [XN, YN]) :- manhattan([XO, YO], [XN, YN], Dist), Dist > 20.

offsets20([X, Y], Offsets) :-
  LX is X - 20, HX is X + 20,
  LY is Y - 20, HY is Y + 20,
  findall([NX, NY], (between(LX, HX, NX), between(LY, HY, NY)), AllOffsets),
  exclude(manhattan20([X, Y]), AllOffsets, Offsets).
