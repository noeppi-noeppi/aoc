...
main :-
  read_input(Obstacles),
  length(Obstacles, NumObstacles),
  LastObstacle is NumObstacles - 1,
  reverse(Obstacles, ReversedObstacles),
  maplist(assertz, ReversedObstacles, ForgetReferences),
  no_solution_after(LastObstacle, ForgetReferences, Idx),
  nth0(Idx, Obstacles, Fact),
  arg(1, Fact, [X, Y]),
  write(X), write(","), write(Y), nl.

no_solution_after(N, [NextForget | RestForget], Result) :-
  erase(NextForget), !,
  (no_solution -> Prev is N - 1, !, no_solution_after(Prev, RestForget, Result) ; Result is N).
no_solution :- new_pq(PQ), new_as_entries(AsEntries, PQ), \+ solve(AsEntries, PQ).
...
read_input(Obstacles) :-
  ...
  close(Fd),
  split_string(File, "\n", "\n", Lines), !,
  maplist(line_fact, Lines, Obstacles).
